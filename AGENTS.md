...
- Keep the Crystal implementation dependency-light. Avoid shard dependencies unless there is a concrete reason; Kemal is currently used for `bon web` routing and ECR-backed templates.
...
- `--verbose` is intentionally long-only because `-v` remains the version shortcut; verbose diagnostics go to stderr with a `[verbose]` prefix so stdout remains stable for dry-run commands and generated simulate paths.
...
- Local install/uninstall helpers live in `bin/bon-install-local` and `bin/bon-uninstall-local` because `bin/install` and `bin/uninstall` are generated mise task stubs. Install prompts before copying `bin/bon-release` to `$HOME/.local/bin/bon`; uninstall prompts before removing only that project-local path and must not remove package-manager installs such as Homebrew-owned binaries.
...
- Homebrew release publication is intentionally two-stage: first commit the source formula to the tap, then build bottles from that tap formula for macOS ARM, macOS x64, and Linux x64, upload `*.bottle.tar.gz` assets to the GitHub release, and use `brew bottle --merge` to add the generated bottle block to the tap formula. Do not hand-author or rename Homebrew bottle archives.
